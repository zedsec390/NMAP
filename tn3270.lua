...
Telnet = {
 ...
 -- used to translate buffer addresses
  ...
  process_write = function ( self, data )
    ...
    if bit.band(select(2, bin.unpack(">C", data:sub(i,i))), 0x40) == 0x40 then
      stdnse.debug(3,"[WCC] Reset")
    end

    if bit.band(select(2, bin.unpack(">C", data:sub(i,i))), 0x02) == 0x02 then
  ...
  w_structured_field = function ( self, wsf_data )
    ...
    local query_options = "8800168186000800f4f100f200f300f400f500f60" ..
                          "0f700000d81870400f0f1f1f2f2f4f40022818582" ..
                          "00071000000000070000000065002500000002b90" ..
                          "0250100f103c30136002e81810300005000180000" ..
                          "01004800010048071000000000000013020001005" ..
                          "000180000010048000100480710001c81a600000b" ..
                          "010000500018005000180b0200000700100007001" ..
                          "0000781880001020016818080818485868788a1a6" ..
                          "a89699b0b1b2b3b4b600088184000a00040006819" ..
                          "90000ffef"
  ...
  send_cursor = function ( self, data )
    ...
    self.output_buffer[0] = bin.pack(">C",self.aids.ENTER) -- what follows is an ENTER
  ...
  send_location = function( self, location, data )
    ...
    local ebcdic_letter = ''
    self.output_buffer = {}
    self.output_buffer[0] = bin.pack(">C",self.aids.ENTER)
  ...
  writeable = function (self)
    ...
    for i = 1,#self.fa_buffer do
      if ( self.fa_buffer[i] ~= "\00" ) and (bit.band(select(2,bin.unpack(">C",self.fa_buffer[i])) , 0x20) ~= 0x20) then
      -- found writeable flag
        for j = i,#self.fa_buffer do
          -- find end of field
          if (bit.band(select(2,bin.unpack(">C",self.fa_buffer[j])) , 0x20) == 0x20) then
